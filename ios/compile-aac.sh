...
do_lipo () {
    ...
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/AAC-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    cp -R $UNI_BUILD_ROOT/build/AAC-armv7/output/include $UNI_BUILD_ROOT/build/universal/
}

#----------
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh tools/do-compile-AAC.sh $FF_TARGET
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh tools/do-compile-AAC.sh $ARCH
    done
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        cd AAC-$ARCH && git clean -xdf && cd -
    done
else
    echo "Usage:"
    echo "  compile-AAC.sh armv7|arm64|i386|x86_64"
    echo "  compile-AAC.sh armv7s (obselete)"
    echo "  compile-AAC.sh lipo"
    echo "  compile-AAC.sh all"
    echo "  compile-AAC.sh clean"
    echo "  compile-AAC.sh check"
    exit 1
fi
